// Simple database update using curl
...
async function updateDatabaseReferences() {
  try {
    ...
    // Get all products using curl
    const { stdout } = await execPromise('curl -s http://localhost:3000/api/test-seller-products');
    ...
    for (const product of products) {
      ...
      if (updated) {
        // Update product using curl
        const updateCommand = `curl -s -X PUT http://localhost:3000/api/test-seller-products/${product.id} -H "Content-Type: application/json" -d '${JSON.stringify({ images: JSON.stringify(images) })}'`;
