...
// Database Product interface (matching Prisma schema)
interface DBProduct {
  id: string;
  title: string;
  price: number;
...
export default function FeaturedProductsDB({ products }: { products: DBProduct[] }) {
  ...
  const featuredProducts = products
    .filter((product: DBProduct) => product.featured)
    .slice(0, 8);

  const handleAddToCart = async (product: DBProduct, e?: React.MouseEvent) => {
    e?.stopPropagation();
  ...
  const handleQuickView = (product: DBProduct, e?: React.MouseEvent) => {
    e?.stopPropagation();
