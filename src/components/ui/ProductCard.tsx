...
export default function ProductCard({
  ...
  return (
    <div
      className={`group relative bg-white rounded-[12px] sm:rounded-[16px] overflow-hidden shadow-[0_8px_24px_rgba(0,0,0,0.04)] hover:shadow-[0_16px_32px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-400 ease-out flex flex-col h-full ${
    ...
    >
      ...
      <Link href={`/product/${(product as any).slug || generateSlug(product.name || product.title || '')}`}>
        <div className="relative overflow-hidden bg-gray-50 cursor-pointer transition-opacity duration-300" style={{ aspectRatio: '1 / 1' }}>
          ...
          <button
            ...
            className="absolute top-2 sm:top-3 right-2 sm:right-3 w-7 h-7 sm:w-8 sm:h-8 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm hover:bg-white hover:scale-110 transition-all duration-300 z-10"
          >
            <Heart className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${productInWishlist ? 'fill-current text-red-500' : 'text-gray-600'}`} />
          </button>
      ...
      {/* Notification Toast */}
      {notification && (
        <div className={`absolute top-4 sm:top-6 right-4 sm:right-6 z-50 px-3 py-2 sm:px-4 sm:py-3 rounded-[8px] sm:rounded-[12px] shadow-lg flex items-center gap-2 sm:gap-3 transition-all duration-300 ${
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-grow bg-white relative">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-baseline gap-0.5">
            ...
            <span className="text-sm sm:text-base font-semibold text-[#2d3436]">
